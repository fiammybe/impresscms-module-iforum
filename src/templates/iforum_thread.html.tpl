...
<table width="100%">
  	<tr>
       		<th>
       		<div class="postedon" style="float: <{$smarty.const._GLOBAL_LEFT}>;"><{$topic_post.post_date}></div>

		<div class="comTitle" style="float: <{$smarty.const._GLOBAL_RIGHT}>;">
  	...
  	<tr>
  		<td class="odd" valign="top">
			...
			<div style="margin-left: auto; margin-right: auto; text-align: center;padding-top: 5px;" class="Userbar">
				<a onclick="jQuery('div.Userrow_right').toggle(400);" style="line-height:auto;" name="Userrow_right" href="#postedon" title="<{$smarty.const._MD_USERSTAT}>">
				<img src="<{$icms_url}>/modules/<{$icms_dirname}>/images/userrow_right.png" /></a>
				<{foreach item=bar from=$topic_post.poster.userbar}>
				<{if $bar}>
				<a class="UserLink" href="javascript:openWithSelfMain('<{$bar.link}>','pmlite',800,680);" title="<{$bar.name}>"><{$bar.image}></a>
				<{/if}>
				<{/foreach}>
			</div>
			</div>
			
			<div class="Userrow_center">
			<div class="comUserName"><{$topic_post.poster.link}></div>
			<{if $topic_post.poster.status}><div class="comUserStatus"><{$topic_post.poster.status}></div><{/if}>
			<div class="clear"></div>
  			<div class="comUserRankText"><{$topic_post.poster.rank.title}><br /><{$topic_post.poster.rank.image}></div>
			<div class="clear"></div>
			</div>

			<div class="Userrow_right" style="display: none">
  			<div class="comUserStat">
				<span class="comUserStatCaption"><{$smarty.const._MD_JOINED}>:</span>
				<{$topic_post.poster.regdate}>

  				<{if $topic_post.poster.from}>
                               	| <span class="comUserStatCaption"><{$smarty.const._MD_FROM}>:</span> <a href="http://<{$icms_langcode}>.wikipedia.org/wiki/<{$topic_post.poster.from}>" rel="external"><{$topic_post.poster.from}></a>  				<{/if}>	
	...
	<tr>
  		<td class="even" valign="top">
			<div class="comTitle"><h3><{$topic_post.post_title}></h3></div>
			...
			<div class="clear"></div>
    			<br />
			<div style="float: <{$smarty.const._GLOBAL_RIGHT}>; padding: 5px; margin-top: 10px;"><small><{$topic_post.post_edit}></small></div>
			<{/if}>
		</td>
  	</tr>
	<tr>
  		<td class="even" valign="bottom">
	...
	<tr>
    		<td class="foot">
			<div style="float: <{$smarty.const._GLOBAL_LEFT}>;" class="transfer">
    	<{foreach item=act from=$topic_post.thread_action}>
		<a href="<{$act.link}>" title="<{$act.name}>"> <{$act.image}></a>
			<{/foreach}>
			</div>
			<div style="float: <{$smarty.const._GLOBAL_RIGHT}>;" class="foot_buttons">
    			<{if $mode gt 1}>
