...
<table width="100%">
  <tr>
       <th style="width: 20%;" align="<{$smarty.const._GLOBAL_LEFT}>">
    ...
    <th align="<{$smarty.const._GLOBAL_RIGHT}>"><div class="comTitle" style="float: <{$smarty.const._GLOBAL_RIGHT}>;"><a href="<{$icms_url}>/modules/<{$icms_dirname}>/viewtopic.php?post_id=<{$topic_post.post_id}>#forumpost<{$topic_post.post_id}>">#<{$topic_post.post_no}></a></div></th>
  </tr>

  <tr>
  	<td style="width: 20%;" class="odd" rowspan="2" valign="top">
  ...
  <tr>
    <td colspan="2" class="even" valign="bottom">
  ...
  <tr>
    <td style="width: 20%;" class="foot">
    	<{foreach item=act from=$topic_post.thread_action}>
				<a href="<{$act.link}>" title="<{$act.name}>"> <{$act.image}></a>
			<{/foreach}>
	</td>
    <td colspan="2" class="foot"><div style="float: <{$smarty.const._GLOBAL_RIGHT}>">
    <{if $mode gt 1}>
