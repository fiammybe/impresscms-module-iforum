...
if (!empty($_POST['submit']))
{
	...
	{
		$sql = "update " . icms::$xoopsDB->prefix("bb_categories") . " set cat_order = " . $cat_orders[$i] . " WHERE cat_id=$cat[$i]";
...
else
{
	...
	$tform = new icms_form_Theme(_AM_IFORUM_SETFORUMORDER, "", "");
	$tform->display();
	...
	foreach ($forums as $forumid => $forum)
	{
		...
		'forum_order' => (int)$forum->getVar('forum_order'),
			'forum_id' => $forumid,
